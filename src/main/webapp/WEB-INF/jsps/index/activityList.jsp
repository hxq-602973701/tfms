...
<div id="bg" class="main">
	<div class="w950">
		<div class="keyword">
			<h4>当前位置：</h4><a href="/">首页</a><span>&nbsp;</span> 活动列表</div>
			<div class="content1">
			  <div class="left">
			  	<!-- 模块管理 -->
			  ...
			  <div class="right">
				<div class="article_wrap">
				  <dl class="article_con">
						...
						<dd><span class="date"><a href="<%=request.getContextPath()%>/${activity.id}/activityShow" target="_blank">${activity.title}</a></span> <span class="time">${activity.createTime}</span></dd>
						</c:forEach>
				  </dl>
				  <div class="pages">
					  <div class="pages"><a>总计:${datas.recordCount}条 每页${datas.pageSize}条 当前第:${datas.currentPage}页</a>   
					  
					  <a href="<%=request.getContextPath()%>/activityList?currentPage=1">首页</a> 
					  <c:if test="${datas.currentPage==1}">
					  <a href="javascript:void(0);">上一页</a> 
					  </c:if>
					  <c:if test="${datas.currentPage!=1}">
					  <a href="<%=request.getContextPath()%>/activityList?currentPage=${datas.currentPage-1}">上一页</a> 
					  </c:if>
					  <c:if test="${datas.currentPage==datas.pageCount}">
					  <a href="javascript:void(0);">下一页</a> 
					  </c:if>
					  <c:if test="${datas.currentPage!=datas.pageCount}">
					  <a href="<%=request.getContextPath()%>/activityList?currentPage=${datas.currentPage+1}">下一页</a> 
					  </c:if>
					  <a href="<%=request.getContextPath()%>/activityList?currentPage=${datas.pageCount}">尾页</a> 
					  </div>
				  </div>
