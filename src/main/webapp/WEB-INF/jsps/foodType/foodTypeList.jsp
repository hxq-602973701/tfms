...
<div class="rightinfo">
    ...
    <table class="tablelist">
        ...
        <tbody>
        	 <c:forEach items="${datas.records}" var="foodtype">
	        <tr>
		        <td>${foodtype.name}</td>
		        <td>
		        	<c:if test="${foodtype.state eq 1}">
		        		<span class="label label-success">启用</span>
		        	</c:if>
		        	<c:if test="${foodtype.state eq 0}">
		        		<span class="label btn-danger">禁用</span>
		        	</c:if>
		        </td>
		        <td>${foodtype.description}</td>
		        <td>
		        	<a href="${foodtype.id}/update" class="tablelink">修改</a>&nbsp; 
		        	<a href="javascript:del('${foodtype.id}')" class="tablelink"> 删除</a>
		        </td>
	        </tr>
	        </c:forEach>
        </tbody>
  ...
  <!-- 分页 开始-->
   <div class="pagin">
        ...
        <ul class="paginList" style="float：left;">
        	<li class="paginItem">
        	<a href="<%=request.getContextPath()%>/foodType/list?currentPage=1">首页</a></li>
        	<c:if test="${datas.currentPage==1}">
        		<li class="paginItem"><a href="javascript:void(0);">上一页</a></li>
        	</c:if>
        	<c:if test="${datas.currentPage!=1}">
        		<li class="paginItem"><a href="<%=request.getContextPath()%>/foodType/list?currentPage=${datas.currentPage-1}">上一页</a></li>
        	</c:if>
	        ...
	        <c:if test="${datas.currentPage!=datas.pageCount}">
	        	<li class="paginItem"><a href="<%=request.getContextPath()%>/foodType/list?currentPage=${datas.currentPage+1}"">下一页</a></li>
	        </c:if>
	        <li class="paginItem"><a href="<%=request.getContextPath()%>/foodType/list?currentPage=${datas.pageCount}">尾页</a></li>
        </ul>
       	<div class="goPage">
...
<script type="text/javascript">
	...
	function goPage()
	{
		...
		window.location.href="${pageContext.request.contextPath}/foodType/list?currentPage="+page;
	}
</script>
</body>
