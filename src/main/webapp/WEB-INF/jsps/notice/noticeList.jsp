...
<div class="rightinfo">
    ...
    <table class="tablelist">
        ...
        <tbody>
        	 <c:forEach items="${datas.records}" var="notice">
	        <tr>
		        <td>${notice.title}</td>
		        ...
		        <td>
		        	...
		        	<c:if test="${notice.state eq 0}">
		        		<span class="label btn-danger">未发布</span>
		        	</c:if>
		        </td>
		        <td>
		        	<a href="${notice.id}/update" class="tablelink">修改</a>&nbsp;   
		        	<a href="javascript:del('${notice.id}')" class="tablelink"> 删除</a>
		        </td>
	        </tr>
	        </c:forEach>
        </tbody>
  ...
  <!-- 分页 开始-->
   <div class="pagin">
        ...
        <ul class="paginList" style="float：left;">
        	<li class="paginItem"><a href="<%=request.getContextPath()%>/notice/list?currentPage=1">首页</a></li>
        	<c:if test="${datas.currentPage==1}">
        		<li class="paginItem"><a href="javascript:void(0);">上一页</a></li>
        	</c:if>
        	<c:if test="${datas.currentPage!=1}">
        		<li class="paginItem"><a href="<%=request.getContextPath()%>/notice/list?currentPage=${datas.currentPage-1}">上一页</a></li>
        	</c:if>
	        ...
	        <c:if test="${datas.currentPage!=datas.pageCount}">
	        	<li class="paginItem"><a href="<%=request.getContextPath()%>/notice/list?currentPage=${datas.currentPage+1}"">下一页</a></li>
	        </c:if>
	        <li class="paginItem"><a href="<%=request.getContextPath()%>/notice/list?currentPage=${datas.pageCount}">尾页</a></li>
        </ul>
       	<div class="goPage">
...
<script type="text/javascript">
	...
	function goPage()
	{
		...
		window.location.href="${pageContext.request.contextPath}/notice/list?currentPage="+page;
	}
</script>
</body>
