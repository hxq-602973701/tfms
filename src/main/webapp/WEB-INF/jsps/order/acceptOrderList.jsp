...
<div class="rightinfo">
    ...
    <table class="tablelist">
        ...
        <tbody>
	        ...
	        <tr>
		        ...
		        <td>${order.phone}</td>
		        <td>
		   			<span class="label btn-danger">正在配送</span>
		        </td>
		        <td>${order.total}</td>
		        <td>${order.date}</td>
		        <td>
		        	<a href="javascript:preview1('${order.id}');" class="tablelink">打印</a>
		        	<a href="${order.id}/cancelOrder" class="tablelink"> 取消</a>
		        	<a href="${order.id}/finishOrder" class="tablelink"> 完成</a>
		        </td>
	        </tr>
	        </c:forEach>
        </tbody>
   ...
   <div class="pagin">
        ...
        <ul class="paginList" style="float：left;">
        	<li class="paginItem"><a href="<%=request.getContextPath()%>/order/acceptOrderList?currentPage=1">首页</a></li>
        	<c:if test="${datas.currentPage==1}">
        		<li class="paginItem"><a href="javascript:void(0);">上一页</a></li>
        	</c:if>
        	<c:if test="${datas.currentPage!=1}">
        		<li class="paginItem"><a href="<%=request.getContextPath()%>/order/acceptOrderList?currentPage=${datas.currentPage-1}">上一页</a></li>
        	</c:if>
	        ...
	        <c:if test="${datas.currentPage!=datas.pageCount}">
	        	<li class="paginItem"><a href="<%=request.getContextPath()%>/order/acceptOrderList?currentPage=${datas.currentPage+1}"">下一页</a></li>
	        </c:if>
	        <li class="paginItem"><a href="<%=request.getContextPath()%>/order/acceptOrderList?currentPage=${datas.pageCount}">尾页</a></li>
        </ul>
       	<div class="goPage">
...
<script type="text/javascript">
	...
	function goPage()
	{
		...
		window.location.href="${pageContext.request.contextPath}/order/acceptOrderList?currentPage="+page;
	}
</script>
</body>
